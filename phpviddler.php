...
class Phpviddler {
	...
	var $viddlerRESTssl = 'https://api.viddler.com/rest/v1/'; // REST URL Version 1.0
	var $parser = true; // Use the included XML parser? Default: true.
        
        // curl options to override
	var $debug = false; // default false, set to true for debug mode on curl calls to app log
        var $curlVerbose = false; // default false, set to true to debug curl calls in std error
        var $doNotSslVerifyPeer = false; // default false, set true to have curl calls not verify ssl peer
        var $curlForceFreshConnect = false; //default false, set to true to always force curl to use a fresh connection
        var $curlMaxRetry = 3; // default 3, maximum number of retries for curl requests, set to 1 to not retry after first attempt
        var $curlConnectionTimeout = 1; // initial number of seconds for curl connection timeout
        var $curlConnectionTimeoutRetryIncrement = 1; // default is 1, set seconds to increase connection timeout by per retry, set to 0 to use the same curlConnectionTimeout each time

        // the current state
        var $curlRetryCount = 0; // the number of retries performed for last curl request
        var $curlError = null; // set to string of last curl error occurrence
        var $curlCurrentConnectionTimeout = 1; // increments as defined by curlConnectionTimeoutRetryIncrement on retries
        ...
        /*
        *  override for application defined logging
        */
        function appLog($msg)
        {
          // create your own extended class of Php5Viddler or PhpViddler
          // and then override this method in the extended class.
          // example for Yii framework
          /*
            class YiiPhp5viddler extends Php5viddler
            {
                function __construct($apiKey=false)
                {
                ...
                //override appLog method
                public function appLog($msg)
                {
                  Yii::log($msg."\n",'error','application.Phpviddler');
                }
            }
          */
        }
...
/*##########  User functions ########### */
	...
	*/
	function user_authenticate($user=null,$pass=null,$getToken=null) {
		...
		$session = $this->sendRequest('viddler.users.auth','user='.$user.'&password='.$pass.'&get_record_token='.$getToken,'get');
...
/*##########  Video functions ########### */
        ...
        function deleteVideo($videoid=null, $sessionid=null) {
		$videoStatus = $this->sendRequest('viddler.videos.delete',array('sessionid'=>$sessionid,'video_id'=>$videoid),'post');
		return $videoStatus;
	}
	...
	*/
	function video_upload($videoInfo=null) {
		...
		$temprest = $this->video_prepareupload($videoInfo['sessionid']);
                $videoDetails = null;
                if (!isset($temprest)) {
                    $this->appLog("video_prepareupload() failed to return result!");
                } else if (!isset($temprest['upload'])) {
                    $this->appLog("video_prepareupload() failed to return upload result!");
                } else if (!isset($temprest['upload']['endpoint'])) {
                    $this->appLog("video_prepareupload() failed to return endpoint result!");
                } else {
                    ...
                    if (!isset($videoDetails)){
                        $this->appLog("video_upload() failed to return videoDetails result!");
                    }
                }
                return $videoDetails;
	}
	...
	*/
	function videos_listbyuser($user=null,$page=null,$per_page=null,$sessionid=null,$tags=null,$sort=null) {
		//$videoList = $this->sendRequest('viddler.videos.getByUser','user='.$user.'&sessionid='.$sessionid.'&page='.$page.'&per_page='.$per_page.'&tags='.$tags.'&sort='.$sort);
                $args = 'user='.$user.'&sessionid='.$sessionid.'&page='.$page.'&per_page='.$per_page;
                if (!is_null($tags)) {
                    $args =  $args.'&tags='.$tags;
                }
                if (!is_null($sort)) {
		...
		$videoList = $this->sendRequest('viddler.videos.getByUser',$args);
		return $videoList;
	}
...
/*##########  Misc. Functions ########### */
	...
	*/
	...
	/ $width(number),$height(number),$autoplay(boolean),$options(array or false),$playerName(string)
	/ returns: HTML
	/ autoplay & options added by tom@punkave.com
    / optional $playerName argument added
	*/
	function video_getEmbed($videoid=null,$type='player',$width=437,$height=370, $autoplay = false, $options = false, $playerName=null) {
            if (!$options) {
              $options = array();
            }
            ...
            $html = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="' .
                    $width . '" height="'.$height.'" id="viddlerplayer-' .
                    $videoid.'"><param name="movie" value="http://www.viddler.com/'.$type.'/'.$videoid.'/" />';
            ...
            $html .= '<param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="true" /><embed src="http://www.viddler.com/' .
                        $type.'/'.$videoid.'/" width="'.$width.'" height="' .
                        $height.'" type="application/x-shockwave-flash" allowScriptAccess="always" allowFullScreen="true"';
            ...
            if (!$playerName) {
                $playerName = 'viddlerplayer-'.$videoid;
            }

            $html .= ' name="'.$playerName.'" wmode="transparent" ></embed></object>';
            return $html;
        }
	...
	*/
        ...
        {
            $this->curlRetryCount = 0;
            $this->curlCurrentConnectionTimeout = $this->curlConnectionTimeout;
            $success = false;

            do
            {
                $response = $this->sendCurlRequest($method,$args,$postmethod);
                if (is_null($this->curlError) && !is_null($response)) {
                    $success = true;
                    break;
                }
                $this->curlCurrentConnectionTimeout += $this->curlConnectionTimeoutRetryIncrement;
                $this->curlRetryCount++;

                if ($this->debug) {
                    $this->appLog("debug ON sendRequest retry #".$this->curlRetryCount." with connection timeout at ".$this->curlCurrentConnectionTimeout." seconds.");
                }

            } while ($this->curlRetryCount < $this->curlMaxRetry);

            if (!$success) {
                $this->appLog("sendRequest failed with curlRetryCount= ".$this->curlRetryCount);
            }
	...
	/ returns: array or xml
	*/
        function sendCurlRequest($method=null,$args=null,$postmethod='get') {
            ...
            // some changes here). We can hope.
                ...
                if ($method == 'viddler.users.auth' || $method == 'viddler.users.register') {
                    $useSSL = true;
                ...
                curl_setopt ($curl_handle, CURLOPT_URL, $reqURL);
                curl_setopt ($curl_handle, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt ($curl_handle, CURLOPT_CONNECTTIMEOUT, $this->curlCurrentConnectionTimeout);
                curl_setopt ($curl_handle, CURLOPT_HEADER, 0);
                curl_setopt ($curl_handle, CURLOPT_TIMEOUT, 0);

                if ($useSSL || $this->curlRetryCount > 0 || $this->curlForceFreshConnect) {
                    curl_setopt ($curl_handle, CURLOPT_FRESH_CONNECT, 1);
                }

                if ($this->doNotSslVerifyPeer) {
                ...
                if ($postmethod == 'post') {
                    ...
                    curl_setopt($curl_handle, CURLOPT_POSTFIELDS, $getArgs);
                }

                $response = curl_exec($curl_handle);

                if (!$response)	{
                    $this->curlError = curl_error($curl_handle);
                    $printargs = print_r($getArgs, true);
                    $this->appLog("Viddler API no response with curl error \ncurl_error: ".$this->curlError."\nurl: ".$reqURL."\nargs: ".$printargs);
                    curl_close($curl_handle);
                    return null;
                }
                ...
                if ($this->debug) {
                    $printargs = print_r($getArgs, true);
                    $printrepsonse = print_r($response, true);
                    $this->appLog("debug ON curl_exec \nurl: ".$reqURL."\nargs: ".$printargs."\nresponse: ".$printrepsonse);
                }
